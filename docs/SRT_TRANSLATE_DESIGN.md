...
```text
srtspeak translate --srt SRC --to en --to pt-BR --out srt_gen
        → srtspeak build-all --map ja=SRC --map en=srt_gen/en/… --map pt=…
```
...
| キャッシュ | **出力 SRT ファイル名キー** `work/translate/by_out/{tgt}__{out_name}.json`。既存出力 SRT からもシード。言語間で共有しない |
...
```bat
srtspeak translate --srt GRAN_TENKU_japan.srt --source-lang ja --to en --to pt-BR --out srt_gen --naming gran_tenku
srtspeak build-all ^
  --map ja=GRAN_TENKU_japan.srt ^
  --map en=srt_gen/en/GRAN_TENKU_en.srt ^
  --map pt=srt_gen/pt-BR/GRAN_TENKU_pt-BR.srt ^
...
```text
path = work/translate/by_out/{target_dir}__{out_srt_name}.json
# 例: work/translate/by_out/en__GRAN_TENKU_en.srt.json
...
| 旧形式 | `work/translate/{srt_hash}/cache_*.json`（sha256 キー）は読まない |
...
- 診断: `SRTSPEAK_GUI_PROGRESS_LOG=1` → `work/gui_progress.log` のみ
...
| 0.8 | **translate cache 再設計**: 出力 SRT ファイル名キー `work/translate/by_out/{tgt}__{out}.json`、index→{src,tgt}、既存 out SRT シード。旧 sha256 キー廃止 |
