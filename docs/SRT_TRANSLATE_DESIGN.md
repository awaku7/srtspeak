...
実装エントリ: `srtspeak translate` / `srtspeak glossary-suggest` / GUI Translate タブ（`core/srt_translate.py`, `core/translate_glossary.py`）。

---

## 1. 現状とギャップ

### 1.1 今の srtspeak（TTS 側）

| できること | 本設計で追加済み |
|------------|------------------|
| 言語ごとの **既存 SRT** → TTS → 尺フィット → 完成 WAV | SRT テキストの **翻訳・生成**（`translate`） |
| `ja_yomi`（ja のみ・漢字→ひらがな・意味非変更） | 言語間の意味転送（Grok Chat） |
| `build-all --map ja=… --map en=…` | ソース 1 本から map 用 SRT を自動生成 + `glossary-suggest` |

前提ワークフロー（現行）:

```text
srtspeak translate --srt SRC --to en --to pt-BR --out out/srt_gen
        → srtspeak build-all --map ja=SRC --map en=out/srt_gen/en/… --map pt=…
```
...
| P8 | **決定論に寄せる** | temperature 低 / 出力ファイル名キー + index の src 一致でヒット。model/glossary 変更時は `--no-cache` |
...
```text
# 推奨: 繰り返し
srtspeak translate --srt src.srt --source-lang ja --to en --to pt-BR --to es --out out/srt_gen

# 任意: カンマ区切りも受け付けるなら同一 list へ
srtspeak translate --srt src.srt --source-lang ja --to en,pt-BR,es --out out/srt_gen
```
...
| キャッシュ | **出力 SRT ファイル名キー** `out/work/translate/by_out/{tgt}__{out_name}.json`。既存出力 SRT からもシード。言語間で共有しない |
...
out/work/translate/by_out/
  {target}__{out_srt_name}.json  # キュー単位。index → {src, tgt}
  translate.log
```

命名方針（実装）:

- **既定 `stem`:** `{source_stem}_{lang}.srt`
- **`gran_tenku`:** `GRAN_TENKU_{lang}.srt` — 既存成果物名と揃え、`build-all` と対応しやすい

`build-all` 接続例:

```bat
srtspeak translate --srt GRAN_TENKU_japan.srt --source-lang ja --to en --to pt-BR --out out/srt_gen --naming gran_tenku
srtspeak build-all ^
  --map ja=GRAN_TENKU_japan.srt ^
  --map en=out/srt_gen/en/GRAN_TENKU_en.srt ^
  --map pt=out/srt_gen/pt-BR/GRAN_TENKU_pt-BR.srt ^
...
第二パスも structured JSON。結果は同一 out キャッシュの index エントリへ上書き。
...
**方針:** キュー単位の sha256 キーは使わない。**出力 SRT ファイル名**を主キーにし、既存の完成訳を即ヒットさせる。

```text
path = out/work/translate/by_out/{target_dir}__{out_srt_name}.json
# 例: out/work/translate/by_out/en__GRAN_TENKU_en.srt.json
# out_srt_name は naming に従う:
#   stem       → {source_stem}_{lang}.srt
#   gran_tenku → GRAN_TENKU_{lang}.srt
```
...
{
  "_version": 2,
  "_out_name": "GRAN_TENKU_en.srt",
  "_target": "en",
  "1": {"src": "<source text>", "tgt": "<translated>"},
  "2": {"src": "...", "tgt": "..."}
}
```

ヒット条件: `cache[index].src == 現在のソース cue.text` かつ `tgt` 非空。

**シード:** キャッシュ未登録でも、出力先 `out_dir/{lang}/{out_name}` が既に存在し structure-lock（件数/index/timing）が通るなら、そのテキストを `src=現ソース` でシード → 再実行は API ゼロ。

| 項目 | 挙動 |
|------|------|
| 無効化 | 同一 index のソース text が変わったエントリのみミス |
| `no_cache` | 読込・出力SRTシードをスキップ。成功後は従来どおり書く |
| 旧形式 | `out/work/translate/{srt_hash}/cache_*.json`（sha256 キー）は読まない |
| model/glossary | ファイル名キーには含めない（再訳は `--no-cache` または該当 index の src 変更） |
...
srtspeak translate
  ...
  4. for each target:
       ...
       b. load by_out cache + seed from existing out SRT
       c. pending cues をバッチ化
       d. Chat API（context + glossary + budget）
          ※ 各バッチ前後で ProgressEvent 必須
       e. validate batch → merge
       f. optional compress pass（ここも progress）
       g. save by_out cache（index→{src,tgt}）
       h. format_srt → write
       i. per-target stats
...
out_dir, work_dir
model=grok-4.5, batch_size=8
glossary_path: Path | None
length_mode: off|hint|enforce|report-only   # default hint
on_empty: fail|keep-source                  # default fail
limit, dry_run, fail_fast=False
prompt_version: int = 1
naming: stem|gran_tenku                     # default stem
heartbeat_s: float = 1.5                    # Chat wait pulse; 0=off
no_cache: bool = False                      # skip read; still write
```
...
- タブ **Translate**: ソース SRT、ターゲット複数チェック、glossary + Suggest、out、length mode、batch size、naming、fail-fast、no-cache、dry-run、実行、進捗
- ソースパス Browse / 確定: `guess_lang_from_filename` → 元言語コンボ
- 共有 API キー: マスク + **Save on this PC** / **Clear saved**（OS keyring; Windows 旧 DPAPI 移行）
- 進捗: 下部共有ラベル + bar（0–1000）。worker → `queue.SimpleQueue` + ~80ms drain（Chat heartbeat で「実行中…」固定しない）
- 診断: `SRTSPEAK_GUI_PROGRESS_LOG=1` → `out/work/gui_progress.log` のみ
- 生成 SRT パスは `translate_report.json` / 結果ダイアログで確認し、Build タブへ手動で渡す
- 設定は `gui_settings.json`（キー平文非保存）
...
| 症状 | 対処 |
|------|------|
| 用語ゆれ | glossary 追加後は `--no-cache` で再訳 |
| 特定キューだけ悪い | SRT を手直し（タイムコード触らない）→ build |
| バッチ丸ごと失敗 | batch-size 下げ、再実行（成功分は by_out cache / 出力 SRT 済み） |
...
| プロンプト変更で訳質が変わる | `--no-cache` で全再訳。必要なら out SRT を退避 |
...
| 0.6 | **実装同期**（0.1.2）: 状態を実装済みに。CLI `translate`/`glossary-suggest`、TranslateConfig 拡張（naming/no_cache/fail_fast/heartbeat）、GUI Translate タブ、doctor 行 |
| 0.7 | パッケージ 0.1.3 同期: keyring/DPAPI（P7・§6.6）、guess_lang 強化、GUI progress queue+coalesce、README 同期 |
| 0.8 | **translate cache 再設計**: 出力 SRT ファイル名キー `out/work/translate/by_out/{tgt}__{out}.json`、index→{src,tgt}、既存 out SRT シード。旧 sha256 キー廃止 |
| 0.9 | パッケージ 0.1.4 同期: 版番号 / `__version__` 整合 |
